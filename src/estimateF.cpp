...
{
	// Read input images
	cv::Mat image1= cv::imread("/home/bobo/code/Mul_view_example/data/church01.jpg");
	cv::Mat image2= cv::imread("/home/bobo/code/Mul_view_example/data/church03.jpg");
	if (!image1.data || !image2.data)
		{
			cout<<"can not read the image"<<endl;
			cout<<"check the image path"<<endl;
			return 0;
		}
    // Display the images
	...
	*/
	...
	selMatches.push_back(matches[260]);
...
while(1)
	...
	cv::Mat fundemental1= cv::findFundamentalMat(
		cv::Mat(selPoints1), // points in first image
		cv::Mat(selPoints2), // points in second image
		CV_FM_7POINT);       // 7-point method

	std::cout << "F-Matrix size= " << fundemental1.rows << "," << fundemental1.cols << std::endl;
	Mat fundemental=fundemental1(cv::Rect(0,0,3,3));
	...
	cout<<"7POINT method calculate the Fundemental"<<endl;
	while(1)
		{
				if (waitKey(10) == 13)
					break;
...
*/
	...
	// Read input images
	image1= cv::imread("/home/bobo/code/Mul_view_example/data/church01.jpg",0);
	image2= cv::imread("/home/bobo/code/Mul_view_example/data/church03.jpg",0);
	...
	cout<<"RANSAC method calculate the Fudemental"<<endl;
	while(1)
		{
				if (waitKey(10) == 13)
					break;
	...
	cout<<"RANSAC method calculate theHomography "<<endl;
	cv::waitKey(0);
	return 0;
	}
